...
> **Protocols with NO states at all** (e.g. Minecraft Bedrock's
> `protocol.json`, unlike Java's): everything, including the packet
> dispatcher, lives directly under the top-level `types:` block — no
> `<state>:` keys anywhere. `Protocol` detects this automatically. In
> that case, pass `state=None, direction=None` to
> `parse_packet`/`serialize_packet`/`read_named`/`write_named`. Bedrock
> also names its dispatcher type `mcpe_packet` instead of `packet`, so
> pass that via `packet_type_name`:
>
> ```python
> proto = Protocol(bedrock_protocol_json, packet_type_name="mcpe_packet")
> data = proto.serialize_packet(None, None, "disconnect", {
>     "reason": 0, "hide_disconnect_reason": False,
>     "message": "bye", "filtered_message": "",
> })
> pkt = proto.parse_packet(None, None, data)
> ```
>
> Calling a stateful protocol with `None`, or a stateless one with a
> real state name, raises `StatelessProtocolError` explaining the
> mismatch instead of a confusing `KeyError`/`UnknownTypeError`.
...
| `compressedNbt` | variable | length-prefixed (`i16`), gzip-compressed NBT — old (pre-1.20.2) Slot Data item-NBT field; `-1` length means absent |
| `restBuffer` | variable | consumes all remaining bytes in the buffer — typical as a packet's last, length-less field |
| `lpVec3`    | 1 or 6+ bytes | quantized, variable-size `{x, y, z}` float triple (modern Minecraft relative/delta position fields); ported field-for-field from `node-minecraft-protocol`'s `lpVec3.js` |
| `raknetMagic` | 16 fixed bytes | RakNet's constant OFFLINE-packet marker; **validates on read** (raises `MagicMismatchError` on mismatch), `write()` ignores its argument and always emits the constant |
| `raknetAddress` | 7 or 29 bytes | RakNet's `SystemAddress` — IPv4 (`{"version": 4, "address": "1.2.3.4", "port": N}`, address bytes bit-inverted on the wire) or IPv6 (`{"version": 6, "address_family", "port", "flow_info", "address" (16 raw bytes), "scope_id"}`) |
...
### 0.4.3
- **RakNet compatibility** (beyond Minecraft): new `raknetMagic`
  primitive (RakNet's fixed 16-byte OFFLINE-packet marker, validates
  on read via new `MagicMismatchError`) and `raknetAddress` composite
  (RakNet's `SystemAddress`, IPv4 7-byte bit-inverted or IPv6
  29-byte). RakNet's `bit` type needs no new primitive -- a
  single-field `bitfield` already covers it byte-for-byte. Validated
  with a real `UnconnectedPing` RakNet packet end to end, plus 2000
  fuzzed IPv4 addresses and manual (non-roundtrip) byte verification
  of the bit inversion.

### 0.4.2
- **Stateless protocol support** (Minecraft Bedrock-style
  `protocol.json`, which declares no states at all): `Protocol(...)`
  now accepts `packet_type_name` (default `"packet"`), and
  `parse_packet`/`serialize_packet`/`read_named`/`write_named` accept
  `state=None, direction=None` when the loaded protocol has no states
  (auto-detected). New `StatelessProtocolError` for state/protocol
  mismatches. Validated against real `minecraft-data`: 52/52 Bedrock
  `protocol.json` versions (0.14 through 1.26.40) round-trip correctly
  through the same high-level API as Java.
- See section 3 for the stateless usage example.

### 0.4.1
- New **`compressedNbt`** primitive: length-prefixed (`i16`), gzip-
  compressed NBT, for the old (pre-1.20.2) Slot Data item-NBT field.
  New `NBTDecompressionError` for corrupted/truncated gzip payloads.
- New **`lpVec3`** primitive: quantized, variable-size `{x, y, z}`
  vec3, ported field-for-field from `node-minecraft-protocol`'s
  `src/datatypes/lpVec3.js` (same constants and bit-packing).
- Found by comparing `protolib`'s primitive set against
  `node-minecraft-protocol`'s custom datatypes
  (`src/datatypes/minecraft.js` /`compiler-minecraft.js`) so a real
  Minecraft `protocol.json` loads without needing hand patches;
  `varlong`, `restBuffer`, `UUID`, `entityMetadataLoop`,
  `topBitSetTerminatedArray` and `registryEntryHolder(Set)` were
  already covered from earlier versions.
- Primitives table (section 5) completed with `compressedNbt`,
  `restBuffer`, and `lpVec3`, which were already implemented but
  missing from the table.
- 8 new tests in `tests/test_primitives.py` covering both new
  primitives (roundtrip, absent case, size boundaries, `size_of`
  consistency, NaN sanitization, corrupted-gzip error path).
