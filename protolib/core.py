...
class Protocol:
    ...
    Not every real protocol declares states this way -- Minecraft
    Bedrock's protocol.json, unlike Java's, has NO top-level state
    keys at all: every type, including the packet dispatcher, lives
    directly under the top-level `types:` block. Protocol detects this
    automatically (no states found -> "stateless" protocol) and then
    parse_packet/serialize_packet/read_named/write_named accept
    state=None, direction=None instead of requiring real names.
    ...
    `packet_type_name` is the name of the top-level dispatcher type
    that parse_packet/serialize_packet read/write (must have exactly
    two fields, `name` and `params` -- see the "packet container with
    mapper+switch" pattern). Defaults to "packet" (Java-style
    protocol.json). Stateless protocols may name it differently --
    e.g. Bedrock's protocol.json calls it "mcpe_packet":

        proto = Protocol(bedrock_json, packet_type_name="mcpe_packet")
        pkt = proto.parse_packet(None, None, raw_bytes)
    ...
    def __init__(self, protocol_source: dict[str, Any] | str, *, fmt: str | None = None,
        ...
        # Some real protocols (Minecraft Bedrock's protocol.json, unlike
        # Java's) declare NO states at all -- everything, including the
        # packet dispatcher type, lives directly under the top-level
        # `types:` block, so self._scopes ends up empty. Detected once
        # here so parse_packet/serialize_packet/read_named/write_named
        # can accept state=None, direction=None for those protocols
        # instead of forcing callers to fabricate a fake state name.
        self._stateless: bool = len(self._scopes) == 0
        ...
        self._composite_handlers: dict[str, Callable] = {
            ...
            "raknetAddress": self._read_raknet_address,
        }
        self._composite_write_handlers: dict[str, Callable] = {
            ...
            "raknetAddress": self._write_raknet_address,
        }
    ...
    def _resolve_scope(self, state: str | None, direction: str | None) -> Scope | None:
        """
        Used by parse_packet/serialize_packet/read_named/write_named.
        Unlike get_scope() (which always expects real state/direction
        names), this also accepts state=None, direction=None for
        protocols that declare no states at all (self._stateless) --
        in that case there's no Scope to return, resolution falls
        straight through to global_types, same as passing scope=None to
        read_type/write_type directly.
        """
        if self._stateless:
            if state is not None or direction is not None:
                raise StatelessProtocolError(stateless=True, state=state, direction=direction)
            return None

        if state is None or direction is None:
            raise StatelessProtocolError(stateless=False, state=state, direction=direction)
        return self.get_scope(state, direction)
    ...
    # ---- raknetAddress (RakNet SystemAddress, IPv4/IPv6) -------------------
    #
    # Wire format (see e.g. https://github.com/vp817/RakNetProtocolDoc,
    # "Address DataType"):
    #
    #   [u8 version]
    #   if version == 4 (7 bytes total):
    #       [u32 BE address (bit-inverted!)][u16 BE port]
    #   if version == 6 (29 bytes total):
    #       [u16 LE address_family][u16 BE port][u32 BE flow_info]
    #       [16 bytes raw address][u32 BE scope_id]
    #
    # The IPv4 "bit-inverted" encoding is a real RakNet quirk (not a
    # transcription error): each of the 4 octets is recorded MSB-to-LSB
    # and then the whole 32-bit value has every bit flipped before being
    # written -- and flipped back on read. This is ported exactly as
    # specified so the bytes match a real RakNet peer; getting this
    # backwards (or skipping the inversion) produces a value that LOOKS
    # like a plausible IP on read but is wrong, so it's covered by
    # dedicated tests (see tests/test_raknet_address.py) with a fixed,
    # known-correct byte sequence rather than only a self-roundtrip.
    #
    # value shape (both directions):
    #   {"version": 4, "address": "1.2.3.4", "port": 19132}
    #   {"version": 6, "address_family": 23, "port": 19133,
    #    "flow_info": 0, "address": b"...16 raw bytes...", "scope_id": 0}

    def _read_raknet_address(self, opts: dict, r: Reader, scope: Scope,
                                fields: dict, root, parent) -> dict:
        version = self.read_type("u8", r, scope, fields, root, parent)
        if version == 4:
            raw = self.read_type("u32", r, scope, fields, root, parent)
            inverted = raw ^ 0xFFFFFFFF
            octets = [(inverted >> shift) & 0xFF for shift in (24, 16, 8, 0)]
            address = ".".join(str(o) for o in octets)
            port = self.read_type("u16", r, scope, fields, root, parent)
            return {"version": 4, "address": address, "port": port}
        if version == 6:
            address_family = self.read_type("lu16", r, scope, fields, root, parent)
            port = self.read_type("u16", r, scope, fields, root, parent)
            flow_info = self.read_type("u32", r, scope, fields, root, parent)
            address = r.read_bytes(16)
            scope_id = self.read_type("u32", r, scope, fields, root, parent)
            return {
                "version": 6, "address_family": address_family, "port": port,
                "flow_info": flow_info, "address": address, "scope_id": scope_id,
            }
        raise InvalidTypeDefinition(
            f"raknetAddress: unsupported version {version!r} (expected 4 or 6)"
        )

    def _write_raknet_address(self, opts: dict, value: dict, w: Writer, scope: Scope,
                                 fields: dict, root, parent) -> None:
        version = value.get("version")
        if version == 4:
            self.write_type("u8", 4, w, scope, fields, root, parent)
            octets = [int(part) for part in value["address"].split(".")]
            if len(octets) != 4:
                raise InvalidTypeDefinition(
                    f"raknetAddress: expected a dotted IPv4 string with 4 octets, "
                    f"got {value['address']!r}"
                )
            raw = 0
            for o in octets:
                raw = (raw << 8) | (o & 0xFF)
            inverted = raw ^ 0xFFFFFFFF
            self.write_type("u32", inverted, w, scope, fields, root, parent)
            self.write_type("u16", value["port"], w, scope, fields, root, parent)
            return
        if version == 6:
            self.write_type("u8", 6, w, scope, fields, root, parent)
            self.write_type("lu16", value["address_family"], w, scope, fields, root, parent)
            self.write_type("u16", value["port"], w, scope, fields, root, parent)
            self.write_type("u32", value["flow_info"], w, scope, fields, root, parent)
            addr = value["address"]
            if len(addr) != 16:
                raise InvalidTypeDefinition(
                    f"raknetAddress: IPv6 address must be exactly 16 raw bytes, "
                    f"got {len(addr)}"
                )
            w.write_bytes(addr)
            self.write_type("u32", value["scope_id"], w, scope, fields, root, parent)
            return
        raise InvalidTypeDefinition(
            f"raknetAddress: expected value['version'] to be 4 or 6, got {version!r}"
        )
    ...
    def parse_packet(self, state: str | None, direction: str | None, data: bytes) -> ParsedPacket:
        scope = self._resolve_scope(state, direction)
        r = Reader(data)
        packet = self.read_type(self._packet_type_name, r, scope, {})
        return ParsedPacket(name=packet["name"], params=packet["params"], bytes_read=r.offset)

    def serialize_packet(self, state: str | None, direction: str | None, name: str, params: dict) -> bytes:
        scope = self._resolve_scope(state, direction)
        w = Writer()
        self.write_type(self._packet_type_name, {"name": name, "params": params}, w, scope, {})
        return w.result()

    # direct access to a named type (bypassing "packet"), useful for
    # tests and for parsing/serializing standalone sub-structures
    # (e.g. a slot, an NBT)
    def read_named(self, state: str | None, direction: str | None, type_name: str, data: bytes) -> Any:
        scope = self._resolve_scope(state, direction)
        r = Reader(data)
        return self.read_type(type_name, r, scope, {})

    def write_named(self, state: str | None, direction: str | None, type_name: str, value: Any) -> bytes:
        scope = self._resolve_scope(state, direction)
        w = Writer()
