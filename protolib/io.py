...
class Reader:
    ...
    def ensure_at(self, offset: int, n: int) -> None:
        """Like ensure(), but checks availability from an arbitrary
        `offset` instead of the reader's current position. Used by
        callers that need to peek back at bytes already consumed (e.g.
        topBitSetTerminatedArray re-checking an entry's first byte)
        without risking a bare IndexError if that offset turns out to
        be at or past the end of the buffer."""
        if n < 0:
            raise ValueError(f"number of bytes to read cannot be negative: {n}")
        available = len(self.buffer) - offset
        if available < n:
            raise BufferUnderrun(offset, n, max(available, 0))
