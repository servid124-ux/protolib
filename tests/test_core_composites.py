...
class TestMapper(unittest.TestCase):
    ...
    def test_invalid_key_raises_with_context_instead_of_bare_valueerror(self):
        # 0.4.0 regression test: a typo'd mapping key (not a valid int
        # or hex string) used to raise a bare ValueError/AttributeError
        # from deep inside int(key, 16)/int(key), with no indication of
        # which mapper or which key caused it. Now raises
        # InvalidMapperKeyError with both.
        proto = make_protocol({
            "myType": ["mapper", {"type": "u8", "mappings": {"0xZZ": "typo_case"}}],
        })
        with self.assertRaises(InvalidMapperKeyError) as ctx:
            proto.read_named("play", "toClient", "myType", b"\x05")
        self.assertEqual(ctx.exception.key, "0xZZ")
...
class TestBitfield(unittest.TestCase):
    ...
    def test_unsigned_overflow_raises_instead_of_silently_wrapping(self):
        # 0.4.0 regression test: before this fix, a value that didn't
        # fit in `size` bits was masked with `& ((1 << size) - 1)`
        # silently, corrupting the packet instead of failing at the
        # actual mistake -- inconsistent with fixed-width ints (which
        # already raise via struct.pack) and fixed-count buffers.
        proto = make_protocol({"myType": ["bitfield", [{"name": "a", "size": 3, "signed": False}]]})
        with self.assertRaises(BitfieldOverflowError):
            proto.write_named("play", "toClient", "myType", {"a": 8})  # max for 3 bits is 7

    def test_signed_overflow_raises_with_correct_range(self):
        proto = make_protocol({"myType": ["bitfield", [{"name": "a", "size": 4, "signed": True}]]})
        # valid range for a signed 4-bit field is [-8, 7]
        proto.write_named("play", "toClient", "myType", {"a": 7})  # ok, boundary
        proto.write_named("play", "toClient", "myType", {"a": -8})  # ok, boundary
        with self.assertRaises(BitfieldOverflowError):
            proto.write_named("play", "toClient", "myType", {"a": 8})
        with self.assertRaises(BitfieldOverflowError):
            proto.write_named("play", "toClient", "myType", {"a": -9})
...
class TestTopBitSetTerminatedArray(unittest.TestCase):
    ...
    def test_zero_byte_item_at_buffer_end_raises_buffer_underrun_not_indexerror(self):
        # 0.4.0 regression test: re-checking the first byte of each
        # entry used to index r.buffer[start_offset] directly with no
        # bounds check. An item_type that consumes 0 bytes (a container
        # with no fields is the simplest way to construct one) right at
        # the end of the buffer left start_offset == len(r.buffer),
        # raising a bare IndexError instead of a proper library error.
        proto = make_protocol({"myType": ["topBitSetTerminatedArray", {"type": ["container", []]}]})
        with self.assertRaises(BufferUnderrun):
            proto.read_named("play", "toClient", "myType", b"")
