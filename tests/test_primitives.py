...
class TestCompressedNbt(unittest.TestCase):
    """compressedNbt: gzip'd, length-prefixed NBT (old Slot Data field),
    ported from node-minecraft-protocol's src/datatypes/minecraft.js."""

    def test_absent_roundtrip(self):
        prim = PRIMITIVES["compressedNbt"]
        got, data = roundtrip(prim, None)
        self.assertIsNone(got)
        self.assertEqual(data, b"\xff\xff")  # i16 -1

    def test_simple_tag_roundtrip(self):
        prim = PRIMITIVES["compressedNbt"]
        tag = {"name": "", "type": "compound", "value": {
            "count": {"type": "byte", "value": 3},
        }}
        got, data = roundtrip(prim, tag)
        self.assertEqual(got, tag)
        # length-prefixed: first 2 bytes are a positive i16 length
        length = int.from_bytes(data[0:2], "big", signed=True)
        self.assertGreater(length, 0)
        self.assertEqual(len(data), 2 + length)

    def test_corrupted_gzip_raises(self):
        from protolib.errors import NBTDecompressionError

        prim = PRIMITIVES["compressedNbt"]
        w = Writer()
        w.write_bytes((5).to_bytes(2, "big", signed=True))
        w.write_bytes(b"\x00\x00\x00\x00\x00")  # not real gzip data
        r = Reader(w.result())
        with self.assertRaises(NBTDecompressionError):
            prim.read(r)


class TestLpVec3(unittest.TestCase):
    """lpVec3: quantized length-prefixed vec3, ported field-for-field
    from node-minecraft-protocol's src/datatypes/lpVec3.js."""

    def test_zero_vector_is_single_byte(self):
        prim = PRIMITIVES["lpVec3"]
        got, data = roundtrip(prim, {"x": 0.0, "y": 0.0, "z": 0.0})
        self.assertEqual(data, b"\x00")
        self.assertEqual(got, {"x": 0.0, "y": 0.0, "z": 0.0})

    def test_small_vector_six_bytes(self):
        prim = PRIMITIVES["lpVec3"]
        value = {"x": 0.5, "y": -0.25, "z": 1.0}
        got, data = roundtrip(prim, value)
        self.assertEqual(len(data), 6)
        # quantization is lossy -- check we're close, not byte-exact
        for axis in ("x", "y", "z"):
            self.assertAlmostEqual(got[axis], value[axis], delta=0.01)

    def test_large_vector_needs_continuation_varint(self):
        prim = PRIMITIVES["lpVec3"]
        value = {"x": 100.0, "y": -50.0, "z": 12.0}
        got, data = roundtrip(prim, value)
        self.assertGreater(len(data), 6)  # scale > 3 -> extra varint byte(s)
        for axis in ("x", "y", "z"):
            self.assertAlmostEqual(got[axis], value[axis], delta=0.1)

    def test_size_of_matches_actual_written_length(self):
        prim = PRIMITIVES["lpVec3"]
        for value in (
            {"x": 0.0, "y": 0.0, "z": 0.0},
            {"x": 0.5, "y": -0.25, "z": 1.0},
            {"x": 100.0, "y": -50.0, "z": 12.0},
        ):
            w = Writer()
            prim.write(value, w)
            self.assertEqual(prim.size_of(value), len(w.result()))

    def test_nan_sanitized_to_zero(self):
        prim = PRIMITIVES["lpVec3"]
        got, _ = roundtrip(prim, {"x": float("nan"), "y": 0.0, "z": 0.0})
